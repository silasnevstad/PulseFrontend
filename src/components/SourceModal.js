...
const SourceModal = ({ isModalOpen, setIsModalOpen, selectedSources, setSelectedSources, fetchUpdate, firstTimer }) => {
    ...
    const sourcesByCategory = sources.reduce((categories, source) => {
        (categories[source.category] = categories[source.category] || []).push(source);
        return categories;
    }, {});
            ...
            {isModalOpen && 
                <div className="source-modal__body">
                    ...
                    {Object.entries(sourcesByCategory).map(([category, categorySources]) => (
                        <div className="source-category" key={category}>
                            {/* <h2>{category}</h2> */}
                            <div className="source-category__buttons">
